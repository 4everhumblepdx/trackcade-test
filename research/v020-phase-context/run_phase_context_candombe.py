...
def main():
    ...
    for n,name in enumerate(names,1):
        ...
        if n%10==0: print(json.dumps({'processed':n,'rows':len(rows),'errors':len(errors),'invariance':len(inv)}),flush=True)
    ...
    summary={'corpus':'Candombe ISMIR2015 — v0.20 long-context phase diagnostics','tracks_discovered':len(names),'tracks_analyzed':len(by_track),'segments_analyzed':len(rows),'errors':len(errors),'canonical_invariance_failures':len(inv),'single_segment_tracks':len(single),'large_half_cycle_helps':len(helps),'large_half_cycle_hurts':len(hurts),'large_help_tracks':[r['track'] for r in helps],'large_hurt_tracks':[r['track'] for r in hurts],'selector_applied':False}
    (args.output/'summary.json').write_text(json.dumps(summary,indent=2)+'\n')
    print(json.dumps(summary,indent=2))
    if len(names)!=35 or len(by_track)!=35 or errors or inv:
        raise SystemExit(1)
