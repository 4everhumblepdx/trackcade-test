...
def load_jsons(root,name):
    out=[]
    for p in sorted(root.rglob(name)): out.append((p,json.loads(p.read_text())))
    return out
...
def main():
    ...
    expected_tracks={r['midi_performance'] for r in expected}
    rows=[]
    ...
    for r in rows:
        r['trigger_bool']=b(r.get('trigger'))
        for k in ('selected_f1_70','half_f1_70','half_minus_selected_f1','delta_medianBeatStrength','max_periodicity_gain','delta_meanBeatConfidence'):
            try: r[k]=float(r[k])
            except: r[k]=float('nan')
    trig=[r for r in rows if r['trigger_bool']]
    deltas=[r['half_minus_selected_f1'] for r in trig if math.isfinite(r['half_minus_selected_f1'])]
    large_helps=sum(d>=.20 for d in deltas);large_hurts=sum(d<=-.20 for d in deltas)
    worst=min(deltas) if deltas else None;mean=sum(deltas)/len(deltas) if deltas else None
    ...
    outrows=[]
    for r in rows:
        x=dict(r);x.pop('trigger_bool',None);outrows.append(x)
    ...
    summary={'corpus':'ASAP v1.2 / MAESTRO v2.0.0 frozen non-pure-three phase holdout','selected_performances':100,'analyzed_performances':len(seen),'segments':len(rows),'errors':len(errors),'canonical_invariance_failures':len(inv),'triggers':len(trig),'large_helps':large_helps,'large_hurts':large_hurts,'small_negative_triggers':sum(-.20<d<0 for d in deltas),'worst_trigger_delta_f1_70':worst,'mean_trigger_delta_f1_70':mean,'sum_trigger_delta_f1_70':sum(deltas) if deltas else 0,'status':status,'criteria':{'min_triggers_for_conclusion':2,'reject_large_hurt_at_or_below':-.20,'reject_worst_below':-.10,'require_positive_mean':True,'require_large_help_at_or_above':.20},'trigger_rows':[{'track':r['track'],'segment':int(r['segment']),'delta_f1_70':r['half_minus_selected_f1'],'selected_f1_70':r['selected_f1_70'],'half_f1_70':r['half_f1_70'],'delta_medianBeatStrength':r['delta_medianBeatStrength'],'max_periodicity_gain':r['max_periodicity_gain'],'delta_meanBeatConfidence':r['delta_meanBeatConfidence']} for r in trig]}
