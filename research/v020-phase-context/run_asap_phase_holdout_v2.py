...
def run_analyzer(runner: Path,raw: Path,sr: int,name: str,debug=False):
    ...
    t=time.perf_counter();q=subprocess.run(['node',str(runner),str(raw),str(sr),'1',name],text=True,capture_output=True,timeout=900,env=env)
...
def decode_crop(source: Path,raw: Path,sr: int,start_s,end_s):
    ...
    if end_s is not None:
        origin=float(start_s) if start_s is not None else 0.0;duration=float(end_s)-origin
...
def main():
    ...
    ap.add_argument('--holdout-json',type=Path,required=True);ap.add_argument('--zip-url',required=True)
    ap.add_argument('--baseline-runner',type=Path,required=True);ap.add_argument('--debug-runner',type=Path,required=True)
    ap.add_argument('--output',type=Path,required=True);ap.add_argument('--shard-index',type=int,required=True);ap.add_argument('--shard-count',type=int,required=True)
    ...
    if sha256_file(args.baseline_runner)!=args.expected_runner_sha: raise SystemExit('FAIL-CLOSED: baseline SHA mismatch')
    selected=json.loads(args.holdout_json.read_text())
    if len(selected)!=100 or len({r['maestro_audio_filename'] for r in selected})!=100: raise SystemExit('FAIL-CLOSED: holdout denominator/source drift')
    assigned=[r for i,r in enumerate(selected) if i%args.shard_count==args.shard_index]
    remote=HTTPRangeReader(args.zip_url);rows=[];errors=[];inv=[];sources=[]
    with zipfile.ZipFile(remote,'r') as zf:
        infos=zf.infolist()
        for num,perf in enumerate(assigned,1):
            track=perf['midi_performance'];source=perf['maestro_audio_filename'];matches=[i for i in infos if i.filename==source or i.filename.endswith('/'+source)]
            if len(matches)!=1: errors.append({'track':track,'error':f'source mapping {source!r} matches={len(matches)}'});continue
            info=matches[0];source_path=raw=None
            try:
                ...
                with wave.open(str(source_path),'rb') as wf: sr=wf.getframerate()
                ...
                base,tb=run_analyzer(args.baseline_runner,raw,sr,track,False);dev,td=run_analyzer(args.debug_runner,raw,sr,track,True)
                ...
                for si,(seg,ctx) in enumerate(zip(tempo,dbg)):
                    ...
                    sg=ctx['selected'];hg=ctx['halfCycle'];sp=candidate_times(float(sg['phase']),bpm,start,end);hp=candidate_times(float(hg['phase']),bpm,start,end)
                    pp,pr,sf=greedy(sp,rseg);hpv,hr,hf=greedy(hp,rseg)
                    sg2,sg3,hg2,hg3=group(sg,2),group(sg,3),group(hg,2),group(hg,3)
                    s2,h2, s3,h3 = sg2.get('periodicity'),hg2.get('periodicity'),sg3.get('periodicity'),hg3.get('periodicity')
                    d2=(float(h2)-float(s2)) if finite(s2,h2) else float('nan');d3=(float(h3)-float(s3)) if finite(s3,h3) else float('nan')
                    sb=int(sg.get('beatCount') or 0);hb=int(hg.get('beatCount') or 0)
                    sw2=int(sg2.get('windows') or 0);hw2=int(hg2.get('windows') or 0);sw3=int(sg3.get('windows') or 0);hw3=int(hg3.get('windows') or 0)
                    raw_pair=finite(d2,d3) and d2<=G2_MAX and d3>=G3_MIN
                    trig=raw_pair and min(sw2,hw2,sw3,hw3)>=MIN_WINDOWS and min(sb,hb)>=MIN_BEATS
                    rows.append({'track':track,'segment':si,'tier':(dev.get('timingGuardrail') or {}).get('tier'),'bpm':bpm,'start':start,'end':end,'reference_beats':len(rseg),
                      'selected_precision70':pp,'selected_recall70':pr,'selected_f1_70':sf,'half_precision70':hpv,'half_recall70':hr,'half_f1_70':hf,'half_minus_selected_f1':hf-sf,
                      'selected_beatCount':sb,'half_beatCount':hb,'selected_g2_windows':sw2,'half_g2_windows':hw2,'selected_g3_windows':sw3,'half_g3_windows':hw3,
                      'selected_g2_periodicity':s2,'half_g2_periodicity':h2,'delta_g2_periodicity':d2,'selected_g3_periodicity':s3,'half_g3_periodicity':h3,'delta_g3_periodicity':d3,
                      'raw_pair_match':raw_pair,'trigger':trig,'canonical_invariant':invariant,'baseline_runtime_s':tb,'debug_runtime_s':td})
            except Exception as e: errors.append({'track':track,'error':str(e)})
            finally:
    ...
    analyzed_tracks=len({r['track'] for r in rows});summary={'shard_index':args.shard_index,'shard_count':args.shard_count,'assigned_tracks':len(assigned),'analyzed_tracks':analyzed_tracks,'segments':len(rows),'raw_pair_matches':sum(bool(r['raw_pair_match']) for r in rows),'triggers':sum(bool(r['trigger']) for r in rows),'errors':len(errors),'canonical_invariance_failures':len(inv),'baseline_runner_sha256':sha256_file(args.baseline_runner)}
