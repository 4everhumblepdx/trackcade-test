...
TRACKS = [
'Albums-Cafe_Paradiso-03','Albums-Chrisanne3-04','Albums-Chrisanne3-05','Albums-Latin_Jam5-12',
'Albums-Latino_Latino-04','Albums-StrictlyDancing_Tango-15','Media-103410','Media-103609',
'Media-103611','Media-104009','Media-104806','Media-105603','Media-106003','Media-106108']
...
def main():
    ...
    for name in TRACKS:
        a=audio.get(name); ann=anns.get(name)
        if not a or not ann: errors.append({'track':name,'error':'missing audio or annotation'}); continue
        refs=beats(ann); sr,ch=ffprobe(a)
        ...
        for si,seg in enumerate(js.get('phaseDebug',[])):
            ...
            for ci,cand in enumerate(seg.get('candidates',[])):
                pred=candidate_times(float(cand['phase']),bpm,start,end); p,r,f=greedy(pred,rseg,.070)
                layer={x['name']:x for x in cand.get('layers',[])}
