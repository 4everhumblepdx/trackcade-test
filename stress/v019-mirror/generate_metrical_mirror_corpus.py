...
BASE_BPMS = [96.0, 108.0, 120.0, 132.0]
...
def add_burst(buf, t, freq, amp, decay=0.055, phase=0.0):
    ...
    for n in range(nmax):
        ...
        # A short tonal/transient burst: enough band identity to create layered evidence,
        # but with a sharp attack so onset layers can still see it.
...
def render(spec, path):
    ...
    # Tiny deterministic bed prevents absolute-silence edge behavior without adding a clock.
    for i in range(n):
        x=i/SR
...
def base_spec(rname, ratio, base_bpm, variant):
    # Four variants per ratio = 32 hard cases.  They differ in prominence, phase,
    # and accent organization while retaining the same two physical clocks.
    variants=[
...
def main():
    ...
    manifest=[]
    idx=0
    for ri,(rname,ratio) in enumerate(RATIOS):
        for variant,base_bpm in enumerate(BASE_BPMS):
